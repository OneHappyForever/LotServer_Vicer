...
function Install()
{
  ...
  URLKernel='https://cdn.jsdelivr.net/gh/OneHappyForever/LotServer_Vicer@master/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://cdn.jsdelivr.net/gh/OneHappyForever/LotServer_Vicer@master/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://cdn.jsdelivr.net/gh/OneHappyForever/LotServer_Vicer@master/raw/master/lotServer.tar"
